export class TerminalUtils {
    public static isTuiActive = false;

    private static log(msg: string) {
        if (!this.isTuiActive) console.log(msg);
    }
    ...
    public static printStep(step: string, details: string): void {
        this.log(`${this.colors.fgYellow}>> ${step}:${this.colors.reset} ${details}`);
    }

    public static printInfo(text: string): void {
        this.log(`${this.colors.dim}${text}${this.colors.reset}`);
    }

    public static printSuccess(text: string): void {
        this.log(`${this.colors.fgGreen}✔ SUCCESS:${this.colors.reset} ${text}`);
    }

    public static printError(text: string): void {
        this.log(`${this.colors.fgRed}✖ ERROR:${this.colors.reset} ${text}`);
    }

    public static printAdvice(advice: string): void {
        this.log(`${this.colors.dim}${this.colors.fgMagenta}${advice}${this.colors.reset}`);
    }

    public static printDivider(char: string = '-', length: number = 50): void {
        this.log(this.colors.dim + char.repeat(length) + this.colors.reset);
    }
    ...
    public static printEcosystemStatus(agents: { name: string, balance: number, debt: number, trades: number, bets: number }[]): void {
        ...
        this.log(`${this.colors.fgCyan}${'NAME'.padEnd(10)} | ${'BALANCE'.padEnd(10)} | ${'DEBT'.padEnd(8)} | ${'TRADES'.padEnd(7)} | ${'BETS'}${this.colors.reset}`);
        this.printDivider('-');
        for (const a of agents) {
            const row = `${a.name.padEnd(10)} | ${a.balance.toFixed(4).padEnd(10)} | ${a.debt.toFixed(2).padEnd(8)} | ${a.trades.toString().padEnd(7)} | ${a.bets}`;
            this.log(row);
        }
        this.log(`${this.colors.bright}=================================================================${this.colors.reset}\n`);
    }
    ...
    public static printPositionAlerts(positions: { marketName: string, side: string, shares: number, entryPrice: number, currentPrice: number, pnl: number, pnlPercent: number }[]): void {
        ...
        for (const p of positions) {
            ...
            this.log(`  ${arrowColor}${arrow}${this.colors.reset} ${this.colors.bright}${p.marketName}${this.colors.reset}`);
            this.log(`    ${p.side === 'YES' ? 'Yes' : 'No'} | ${p.shares.toFixed(1)} shares`);
            this.log(`    Entry: $${p.entryPrice.toFixed(3)} -> Now: $${p.currentPrice.toFixed(3)}`);
            this.log(`    PnL: ${pnlColor}${p.pnl >= 0 ? '+' : ''}$${p.pnl.toFixed(2)}${this.colors.reset} (${pnlColor}${p.pnlPercent >= 0 ? '+' : ''}${p.pnlPercent.toFixed(1)}%${this.colors.reset})\n`);
        }
    }

    public static printMaverickLogo(): void {
        this.log(this.colors.bright + this.colors.fgCyan + `
   __  __                       _      _
  |  \\/  |                     (_)    | |
  ...
  |_|  |_|\\__,_|  \\_/  \\___||_|   |_| |_|\\_\\
        ` + this.colors.reset);
        this.log(this.colors.dim + "      Fine Terminal Multi-Agent Ecosystem" + this.colors.reset + "\n");
    }
}
